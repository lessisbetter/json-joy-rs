...
| `json-crdt/model/api/*` (`ModelApi`, `NodeApi`, finder/proxy/fanout/events) | `crates/json-joy-core/src/model_api.rs` | partial | partial | partial | no | yes | Native API baseline now includes `from_patches`, `apply_batch`, path `find/read/select`, root `diff` + `merge`, core mutators (`set`, `obj_put`, `arr_push`, `str_ins`), tolerant JSON-Patch-style `add/replace/remove/op` helpers, change fanout hooks (`on_change`/`off_change`) with local-vs-remote origin tagging, path-bound `NodeHandle` proxy-style mutation/read composition, and typed wrappers (`as_obj/as_arr/as_str/as_val/as_bin/as_vec/as_con`). Remaining gap is generated proxy tree semantics (`s`/proxy path typing) and extension-specific typed APIs. |
