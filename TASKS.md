...
- [~] Add explicit matrix/test-port coverage for `json-crdt/model/api/*` (`ModelApi`, `NodeApi`, finder/proxy/events).
  Baseline added in `crates/json-joy-core/src/model_api.rs` + `crates/json-joy-core/tests/upstream_port_model_api_matrix.rs` and `crates/json-joy-core/tests/upstream_port_model_api_events_matrix.rs` (`from_patches`, `apply_batch`, `find/read/select`, core mutators, tolerant `add/replace/remove/op`, `on_change`/`off_change`, local-vs-remote change origin). Proxy/typed NodeApi parity remains.
