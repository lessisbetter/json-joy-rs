...
| `json-crdt/model/Model.ts` decode/view | `crates/json-joy-core/src/model/mod.rs` | partial | yes | yes | partial | yes | Decode/view parity is expanded and fixture-backed (`model_roundtrip >= 110`, `model_decode_error >= 35`), with broader model semantics still pending. |
| `json-crdt/model` apply semantics + node graph | `crates/json-joy-core/src/model_runtime/mod.rs` | partial | yes | yes | partial | yes | Runtime graph replay coverage is expanded (`model_apply_replay >= 140`) with invariant validation and debug-build invariant enforcement during apply; remaining work is mainly broader differential depth and primitive-level RGA decomposition. |
...
| `json-crdt/model/Model.ts` higher-level lifecycle (`fromPatches`, `applyBatch`, schema-aware `load`) | partial across `model.rs`, `model_runtime/mod.rs`, `model_api/mod.rs`, `less_db_compat.rs` | partial | partial | partial | partial | yes | `from_patches`/`apply_batch`/load-session semantics are fixture-backed at expanded depth (`model_lifecycle_workflow >= 60` + `model_lifecycle_from_fixtures.rs`); schema-aware model typing remains out of current runtime-core scope. |
| `json-crdt/codec/structural/binary/*` encode | `crates/json-joy-core/src/model/mod.rs` (+ runtime graph encoder in `model_runtime/encode.rs`) | partial | partial | yes | partial | yes | Logical structural encoding follows upstream `ClockEncoder` + json-pack CBOR semantics with expanded canonical coverage (`model_canonical_encode >= 30`); roundtrip-decode parity remains green across `tests/upstream_port_model_encode_matrix.rs`. |
