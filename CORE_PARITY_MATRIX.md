...
| `json-crdt/model/Model.ts` higher-level lifecycle (`fromPatches`, `applyBatch`, schema-aware `load`) | partial across `model.rs`, `model_runtime/mod.rs`, `model_api/mod.rs`, `less_db_compat.rs` | native | yes | yes | yes | yes | `from_patches`/`apply_batch`/load-session lifecycle semantics are fixture-backed (`model_lifecycle_workflow >= 60`) and run native in production paths. |
