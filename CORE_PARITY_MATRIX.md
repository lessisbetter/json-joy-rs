...
| `json-crdt/model/api/*` (`ModelApi`, `NodeApi`, finder/proxy/fanout/events) | `crates/json-joy-core/src/model_api.rs` | partial | partial | partial | no | yes | Native API baseline now includes `from_patches`, `apply_batch`, path `find/read/select` and pointer-path variants (escaped tokens and `-` append), root `diff` + `merge`, core mutators (`set`, `obj_put`, `arr_push`, `str_ins`), tolerant JSON-Patch-style `add/replace/remove/op` helpers including length-aware remove spans, per-patch and batch fanout hooks (`on_change`/`off_change`, `on_changes`/`off_changes`) with local-vs-remote origin tagging, path-bound `NodeHandle` proxy-style mutation/read composition, and typed wrappers (`as_obj/as_arr/as_str/as_val/as_bin/as_vec/as_con`). Remaining gap is generated proxy tree semantics (`s`/proxy path typing) and extension-specific typed APIs. |
