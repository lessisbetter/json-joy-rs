...
|---|---|---|---|---|---|---|---|
| `json-crdt-patch/Patch.ts` + binary codec | `crates/json-joy-core/src/patch.rs` | partial | yes | yes | no | yes | Decode parity is broad; production-side patch construction/encode API is still limited. |
| `json-crdt-patch/PatchBuilder.ts` + operations/clock | `crates/json-joy-core/src/patch_builder.rs` | missing | no | partial | no | yes | M6 Slice 1 deliverable. |
| `json-crdt/model/Model.ts` decode/view | `crates/json-joy-core/src/model.rs` | partial | yes | yes | no | yes | Decode/view parity achieved for fixture corpus; broader model semantics pending. |
| `json-crdt/model` apply semantics + node graph | `crates/json-joy-core/src/model_runtime.rs` | partial | partial | yes | no | yes | Runtime graph exists but still optimized for current corpus and needs normalization hardening. |
...
| `json-crdt-diff/JsonCrdtDiff.ts` | `crates/json-joy-core/src/diff_runtime.rs` | bridge | yes | yes | no | no | Runtime diff currently subprocess-delegated to Node oracle. |
| less-db compatibility lifecycle apply | `crates/json-joy-core/src/less_db_compat.rs` | bridge | yes | yes | no | no | `apply_patch` still subprocess-delegated. |
