...
pub fn next2<N: Node2>(arena: &[N], node: u32) -> Option<u32> {
    if let Some(r) = get_r2(arena, node) {
        let mut curr = r;
        while let Some(l) = get_l2(arena, curr) { curr = l; }
        return Some(curr);
    }
    let mut curr = node;
...
pub fn prev2<N: Node2>(arena: &[N], node: u32) -> Option<u32> {
    if let Some(l) = get_l2(arena, node) {
        let mut curr = l;
        while let Some(r) = get_r2(arena, curr) { curr = r; }
        return Some(curr);
    }
    let mut curr = node;
    let mut p = get_p2(arena, node);
...
fn insert_right2<N: Node2>(arena: &mut Vec<N>, node: u32, p: u32) {
    ...
    set_r2(arena, node, r);
    if let Some(r) = r { set_p2(arena, r, Some(node)); }
}
...
fn insert_left2<N: Node2>(arena: &mut Vec<N>, node: u32, p: u32) {
    ...
    set_p2(arena, node, Some(p));
    set_l2(arena, node, l);
    if let Some(l) = l { set_p2(arena, l, Some(node)); }
}
...
{
    let Some(root_idx) = root else { return Some(node); };
    let mut curr = root_idx;
    loop {
        let cmp = comparator(&arena[node as usize], &arena[curr as usize]);
        let next = if cmp.is_lt() { get_l2(arena, curr) } else { get_r2(arena, curr) };
        match next {
            None => {
                if cmp.is_lt() { insert_left2(arena, node, curr); }
                else           { insert_right2(arena, node, curr); }
                break;
            }
            Some(n) => curr = n,
        }
...
pub fn remove2<N: Node2>(arena: &mut Vec<N>, root: Option<u32>, node: u32) -> Option<u32> {
    ...
    match (l, r) {
        (None, None) => {
            if let Some(p) = p {
                if get_l2(arena, p) == Some(node) { set_l2(arena, p, None); }
                else                               { set_r2(arena, p, None); }
            }
        ...
        (Some(l), Some(r)) => {
            ...
            while let Some(rr) = get_r2(arena, most_right) { most_right = rr; }
            set_r2(arena, most_right, Some(r));
            set_p2(arena, r, Some(most_right));
            if let Some(p) = p {
                if get_l2(arena, p) == Some(node) { set_l2(arena, p, Some(l)); }
                else                               { set_r2(arena, p, Some(l)); }
        ...
        _ => {
            ...
            if let Some(p) = p {
                if get_l2(arena, p) == Some(node) { set_l2(arena, p, Some(child)); }
                else                               { set_r2(arena, p, Some(child)); }
                root
            } else {
                Some(child)
            }
...
mod tests {
    ...
    struct N {
        key: u64,
        p2: Option<u32>, l2: Option<u32>, r2: Option<u32>,
    }
    ...
    fn cmp(a: &N, b: &N) -> std::cmp::Ordering { a.key.cmp(&b.key) }

    fn node(key: u64) -> N { N { key, ..Default::default() } }
    ...
    fn remove2_leaf() {
        ...
        let mut root: Option<u32> = None;
        for i in 0..arena.len() as u32 { root = insert2(&mut arena, root, i, cmp); }
