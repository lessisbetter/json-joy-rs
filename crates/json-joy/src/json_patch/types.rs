...
pub enum Op {
    ...
    Replace { path: Path, value: Value, old_value: Option<Value> },
    Copy    { path: Path, from: Path },
    Move    { path: Path, from: Path },
    Test    { path: Path, value: Value, not: bool },

    // ── Extended operations ───────────────────────────────────────────────
    StrIns  { path: Path, pos: usize, str_val: String },
    StrDel  { path: Path, pos: usize, str_val: Option<String>, len: Option<usize> },
    Flip    { path: Path },
    Inc     { path: Path, inc: f64 },
    Split   { path: Path, pos: usize, props: Option<Value> },
    Merge   { path: Path, pos: usize, props: Option<Value> },
    Extend  { path: Path, props: Map<String, Value>, delete_null: bool },

    // ── First-order predicate operations ─────────────────────────────────
    Defined      { path: Path },
    Undefined    { path: Path },
    Contains     { path: Path, value: String, ignore_case: bool },
    Ends         { path: Path, value: String, ignore_case: bool },
    Starts       { path: Path, value: String, ignore_case: bool },
    In           { path: Path, value: Vec<Value> },
    Less         { path: Path, value: f64 },
    More         { path: Path, value: f64 },
    Matches      { path: Path, value: String, ignore_case: bool },
    TestType     { path: Path, type_vals: Vec<JsonPatchType> },
    TestString   { path: Path, pos: usize, str_val: String, not: bool },
    TestStringLen{ path: Path, len: usize, not: bool },
    Type         { path: Path, value: JsonPatchType },

    // ── Second-order predicate operations ─────────────────────────────────
    And { path: Path, ops: Vec<Op> },
    Not { path: Path, ops: Vec<Op> },
    Or  { path: Path, ops: Vec<Op> },
}
...
impl Op {
    ...
    pub fn is_predicate(&self) -> bool {
        matches!(self,
            Op::Test { .. } | Op::Defined { .. } | Op::Undefined { .. }
            | Op::Contains { .. } | Op::Ends { .. } | Op::Starts { .. }
            | Op::In { .. } | Op::Less { .. } | Op::More { .. }
            | Op::Matches { .. } | Op::TestType { .. } | Op::TestString { .. }
            | Op::TestStringLen { .. } | Op::Type { .. }
            | Op::And { .. } | Op::Not { .. } | Op::Or { .. }
        )
    }
}
...
impl Default for ApplyPatchOptions {
    fn default() -> Self { Self { mutate: false } }
}
