...
pub fn decode(data: &Value) -> Patch {
    ...
    let ops = obj.get("ops").and_then(|v| v.as_array()).map(|a| a.as_slice()).unwrap_or(&[]);
    for op_val in ops {
        let op_obj = match op_val.as_object() { Some(o) => o, None => continue };
        let op_name = match op_obj.get("op").and_then(|v| v.as_str()) { Some(s) => s, None => continue };

        match op_name {
            "new_con" => {
                if op_obj.get("timestamp").and_then(|v| v.as_bool()).unwrap_or(false) {
            ...
            "ins_obj" => {
                let obj = decode_id(op_obj.get("obj").unwrap_or(&Value::Null));
                let value = op_obj.get("value").and_then(|v| v.as_array()).map(|a| a.as_slice()).unwrap_or(&[]);
                let tuples: Vec<(String, Ts)> = value.iter()
                    .filter_map(|pair| {
            ...
            "ins_vec" => {
                let obj = decode_id(op_obj.get("obj").unwrap_or(&Value::Null));
                let value = op_obj.get("value").and_then(|v| v.as_array()).map(|a| a.as_slice()).unwrap_or(&[]);
                let tuples: Vec<(u8, Ts)> = value.iter()
                    .filter_map(|pair| {
                        let arr = pair.as_array()?;
            ...
            "ins_str" => {
                let obj = decode_id(op_obj.get("obj").unwrap_or(&Value::Null));
                let after = op_obj.get("after")
                    .map(decode_id)
                    .unwrap_or(obj);
                let data = op_obj.get("value")
                    .and_then(|v| v.as_str())
                    .unwrap_or("")
                    .to_owned();
            ...
            "ins_arr" => {
                let obj = decode_id(op_obj.get("obj").unwrap_or(&Value::Null));
                let after = op_obj.get("after").map(decode_id).unwrap_or(obj);
                let values = op_obj.get("values").and_then(|v| v.as_array()).map(|a| a.as_slice()).unwrap_or(&[]);
            ...
            "del" => {
                let obj = decode_id(op_obj.get("obj").unwrap_or(&Value::Null));
                let what_arr = op_obj.get("what").and_then(|v| v.as_array()).map(|a| a.as_slice()).unwrap_or(&[]);
                let what: Vec<crate::json_crdt_patch::clock::Tss> = what_arr.iter()
                    .filter_map(|s| {
                        let arr = s.as_array()?;
