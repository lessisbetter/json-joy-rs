...
const MODEL_API_WORKFLOW_XFAILS: &[&str] = &[];
...
fn run_model_api_workflow(xfails: &[&str]) {
    let scenario = "model_api_workflow";
    ...
    for entry in entries {
        ...
        let expected = select_expected_fields(&expected_full, &["steps", "final_view_json"]);
...
#[test]
fn model_api_workflow_matches_upstream_fixtures() {
    run_model_api_workflow(MODEL_API_WORKFLOW_XFAILS);
}
