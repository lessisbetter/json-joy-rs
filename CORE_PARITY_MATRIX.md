...
| `json-crdt/codec/structural/binary/*` encode | `crates/json-joy-core/src/model.rs` (+ new encoder module) | partial | partial | yes | no | yes | Full native model-state encoding from runtime graph is not complete. Current apply-replay binary parity inventory: 24/30 (`tests/upstream_port_model_encode_matrix.rs`). |
