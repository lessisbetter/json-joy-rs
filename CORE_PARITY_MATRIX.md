...
| `json-crdt/model/api/*` (`ModelApi`, `NodeApi`, finder/proxy/fanout/events) | `crates/json-joy-core/src/model_api.rs` | partial | partial | partial | no | yes | Native API baseline now includes `from_patches`, `apply_batch`, path `find/read/select`, core mutators (`set`, `obj_put`, `arr_push`, `str_ins`), tolerant JSON-Patch-style `add/replace/remove/op` helpers, and change fanout hooks (`on_change`/`off_change`) with local-vs-remote origin tagging. Proxy/typed NodeApi parity is still pending. |
