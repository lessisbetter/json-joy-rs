...
fn encode_path(path: &[String]) -> Value {
    Value::String(if path.is_empty() {
        ...
        format!("/{}", path.iter().map(|s| s.replace('~', "~0").replace('/', "~1")).collect::<Vec<_>>().join("/"))
    })
}

fn decode_path(v: &Value) -> Result<Vec<String>, PatchError> {
    let s = v.as_str().ok_or_else(|| PatchError::InvalidOp("path must be a string".into()))?;
    Ok(json_joy_json_pointer::parse_json_pointer(s))
}

fn decode_type(v: &Value) -> Result<JsonPatchType, PatchError> {
    let s = v.as_str().ok_or_else(|| PatchError::InvalidOp("type must be a string".into()))?;
...
fn decode_ops(arr: &Value) -> Result<Vec<Op>, PatchError> {
    let arr = arr.as_array().ok_or_else(|| PatchError::InvalidOp("ops must be array".into()))?;
...
pub fn to_json(op: &Op) -> Value {
    match op {
        ...
        Op::Remove { path, old_value } => {
            ...
            if let Some(ov) = old_value { m.insert("oldValue".into(), ov.clone()); }
            Value::Object(m)
        }
        ...
        Op::Test { path, value, not } => {
            ...
            m.insert("path".into(), encode_path(path));
            m.insert("value".into(), value.clone());
            if *not { m.insert("not".into(), json!(true)); }
            Value::Object(m)
        }
        ...
        Op::StrDel { path, pos, str_val, len } => {
            ...
            if let Some(s) = str_val { m.insert("str".into(), json!(s)); }
            if let Some(l) = len { m.insert("len".into(), json!(l)); }
            Value::Object(m)
        }
        ...
        Op::Split { path, pos, props } => {
            ...
            m.insert("path".into(), encode_path(path));
            m.insert("pos".into(), json!(pos));
            if let Some(p) = props { m.insert("props".into(), p.clone()); }
            Value::Object(m)
        }
        ...
        Op::Extend { path, props, delete_null } => {
            ...
            if *delete_null { m.insert("deleteNull".into(), json!(true)); }
            Value::Object(m)
        }
        ...
        Op::Contains { path, value, ignore_case } => {
            ...
            if *ignore_case { m.insert("ignore_case".into(), json!(true)); }
            Value::Object(m)
        }
...
pub fn from_json(v: &Value) -> Result<Op, PatchError> {
    let obj = v.as_object().ok_or_else(|| PatchError::InvalidOp("operation must be an object".into()))?;
    let op_str = obj.get("op")
        .and_then(|v| v.as_str())
    ...
    match op_str {
        ...
        "copy" => {
            let from = decode_path(obj.get("from").ok_or_else(|| PatchError::InvalidOp("copy requires 'from'".into()))?)?;
            Ok(Op::Copy { path, from })
        }
        "move" => {
            let from = decode_path(obj.get("from").ok_or_else(|| PatchError::InvalidOp("move requires 'from'".into()))?)?;
        ...
        "str_ins" => {
            let pos = obj.get("pos").and_then(|v| v.as_u64()).ok_or_else(|| PatchError::InvalidOp("str_ins requires 'pos'".into()))? as usize;
            let str_val = obj.get("str").and_then(|v| v.as_str()).ok_or_else(|| PatchError::InvalidOp("str_ins requires 'str'".into()))?.to_string();
            Ok(Op::StrIns { path, pos, str_val })
        }
        "str_del" => {
            let pos = obj.get("pos").and_then(|v| v.as_u64()).ok_or_else(|| PatchError::InvalidOp("str_del requires 'pos'".into()))? as usize;
            let str_val = obj.get("str").and_then(|v| v.as_str()).map(|s| s.to_string());
            let len = obj.get("len").and_then(|v| v.as_u64()).map(|l| l as usize);
            Ok(Op::StrDel { path, pos, str_val, len })
        }
        "flip" => Ok(Op::Flip { path }),
        "inc" => {
            let inc = obj.get("inc").and_then(|v| v.as_f64()).ok_or_else(|| PatchError::InvalidOp("inc requires 'inc'".into()))?;
            Ok(Op::Inc { path, inc })
        }
        "split" => {
            let pos = obj.get("pos").and_then(|v| v.as_u64()).ok_or_else(|| PatchError::InvalidOp("split requires 'pos'".into()))? as usize;
            let props = obj.get("props").cloned();
            Ok(Op::Split { path, pos, props })
        }
        "merge" => {
            let pos = obj.get("pos").and_then(|v| v.as_u64()).ok_or_else(|| PatchError::InvalidOp("merge requires 'pos'".into()))? as usize;
            let props = obj.get("props").cloned();
        ...
        "extend" => {
            let props = obj.get("props")
                ...
                .clone();
            let delete_null = obj.get("deleteNull").and_then(|v| v.as_bool()).unwrap_or(false);
            Ok(Op::Extend { path, props, delete_null })
        }
        "defined"   => Ok(Op::Defined { path }),
        "undefined" => Ok(Op::Undefined { path }),
        "contains" => {
            let value = obj.get("value").and_then(|v| v.as_str()).ok_or_else(|| PatchError::InvalidOp("contains requires 'value'".into()))?.to_string();
            let ignore_case = obj.get("ignore_case").and_then(|v| v.as_bool()).unwrap_or(false);
            Ok(Op::Contains { path, value, ignore_case })
        }
        "ends" => {
            let value = obj.get("value").and_then(|v| v.as_str()).ok_or_else(|| PatchError::InvalidOp("ends requires 'value'".into()))?.to_string();
            let ignore_case = obj.get("ignore_case").and_then(|v| v.as_bool()).unwrap_or(false);
            Ok(Op::Ends { path, value, ignore_case })
        }
        "starts" => {
            let value = obj.get("value").and_then(|v| v.as_str()).ok_or_else(|| PatchError::InvalidOp("starts requires 'value'".into()))?.to_string();
            let ignore_case = obj.get("ignore_case").and_then(|v| v.as_bool()).unwrap_or(false);
        ...
        "less" => {
            let value = obj.get("value").and_then(|v| v.as_f64()).ok_or_else(|| PatchError::InvalidOp("less requires 'value'".into()))?;
            Ok(Op::Less { path, value })
        }
        "more" => {
            let value = obj.get("value").and_then(|v| v.as_f64()).ok_or_else(|| PatchError::InvalidOp("more requires 'value'".into()))?;
            Ok(Op::More { path, value })
        }
        "matches" => {
            let value = obj.get("value").and_then(|v| v.as_str()).ok_or_else(|| PatchError::InvalidOp("matches requires 'value'".into()))?.to_string();
            let ignore_case = obj.get("ignore_case").and_then(|v| v.as_bool()).unwrap_or(false);
        ...
        "test_string" => {
            let pos = obj.get("pos").and_then(|v| v.as_u64()).ok_or_else(|| PatchError::InvalidOp("test_string requires 'pos'".into()))? as usize;
            let str_val = obj.get("str").and_then(|v| v.as_str()).ok_or_else(|| PatchError::InvalidOp("test_string requires 'str'".into()))?.to_string();
            let not = obj.get("not").and_then(|v| v.as_bool()).unwrap_or(false);
            Ok(Op::TestString { path, pos, str_val, not })
        }
        "test_string_len" => {
            let len = obj.get("len").and_then(|v| v.as_u64()).ok_or_else(|| PatchError::InvalidOp("test_string_len requires 'len'".into()))? as usize;
            let not = obj.get("not").and_then(|v| v.as_bool()).unwrap_or(false);
            Ok(Op::TestStringLen { path, len, not })
        }
        "type" => {
            let value = decode_type(obj.get("value").ok_or_else(|| PatchError::InvalidOp("type requires 'value'".into()))?)?;
            Ok(Op::Type { path, value })
        }
        "and" => {
            let ops = decode_ops(obj.get("apply").ok_or_else(|| PatchError::InvalidOp("and requires 'apply'".into()))?)?;
            Ok(Op::And { path, ops })
        }
        "not" => {
            let ops = decode_ops(obj.get("apply").ok_or_else(|| PatchError::InvalidOp("not requires 'apply'".into()))?)?;
            Ok(Op::Not { path, ops })
        }
        "or" => {
            let ops = decode_ops(obj.get("apply").ok_or_else(|| PatchError::InvalidOp("or requires 'apply'".into()))?)?;
            Ok(Op::Or { path, ops })
        }
...
trait PipeOk {
    fn pipe_ok(self) -> Result<Self, PatchError> where Self: Sized;
}
impl PipeOk for Vec<String> {
    fn pipe_ok(self) -> Result<Self, PatchError> { Ok(self) }
}
...
pub fn from_json_patch(v: &Value) -> Result<Vec<Op>, PatchError> {
    let arr = v.as_array().ok_or_else(|| PatchError::InvalidOp("patch must be an array".into()))?;
    arr.iter().map(from_json).collect()
}
