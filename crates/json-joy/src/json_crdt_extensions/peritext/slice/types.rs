...
impl From<i64> for TypeTag {
    fn from(n: i64) -> Self { TypeTag::Int(n) }
}
impl From<&str> for TypeTag {
    fn from(s: &str) -> Self { TypeTag::Str(s.to_string()) }
}
impl From<String> for TypeTag {
    fn from(s: String) -> Self { TypeTag::Str(s) }
}
    ...
    pub fn from_pack(pv: &PackValue) -> Option<Self> {
        match pv {
            PackValue::Array(items) => {
                let steps: Option<Vec<TypeTag>> =
                    items.iter().map(TypeTag::from_pack).collect();
...
impl From<i64> for SliceType {
    fn from(n: i64) -> Self { SliceType::Simple(TypeTag::Int(n)) }
}
impl From<&str> for SliceType {
    fn from(s: &str) -> Self { SliceType::Simple(TypeTag::Str(s.to_string())) }
}
impl From<String> for SliceType {
    fn from(s: String) -> Self { SliceType::Simple(TypeTag::Str(s)) }
}
impl From<TypeTag> for SliceType {
    fn from(tag: TypeTag) -> Self { SliceType::Simple(tag) }
}
